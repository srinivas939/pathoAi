...
The **frontend** of PathoAI is built as a single-page React 19 application with full mobile responsiveness, offline-first state resilience, and native Capacitor shell support for Android compilation.
...
- Edit React components inside `src/components/screens/`.
- Test real-time changes in browser via `npm run dev` at `http://localhost:3000`.
- Build static assets for native mobile app:
  ```bash
  npm run build
  npm run cap:sync
  ```
